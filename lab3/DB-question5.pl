...
answer5-1-1 :- schema(R), fds(F), candkey(R,F, [m]).
answer5-1-2 :- schema(R), fds(F), candkey(R,F, [m, y]).
answer5-1-3 :- schema(R), fds(F), candkey(R,F, [m, c]).

answer5-2 :- schema(R), fds(F), is3NF(R,F).
answer5-3 :- schema(R), fds(F), isBCNF(R,F).

answer5-4 :- schema(R), fds(F), decomp(D), ljd(R,F,D).
answer5-5 :- schema(R), fds(F), decomp(D), fpd(R,F,D).